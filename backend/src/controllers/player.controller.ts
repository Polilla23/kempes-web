...
export class PlayerController {
  ...
  async create(req: FastifyRequest, reply: FastifyReply) {
    ...
    try {
      await this.playerService.createPlayer(body)

      return reply.status(200).send({ message: 'Player created successfully.' })
    } catch (error) {
  ...
  async update(req: FastifyRequest<{ Params: { id: string }; Body: Partial<Player> }>, reply: FastifyReply) {
    ...
    const { id } = req.params
    try {
      await this.playerService.updatePlayer(id, data)

      return reply.status(200).send({ message: 'Player updated successfully.' })
    } catch (error) {
  ...
  async delete(req: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) {
    ...
    try {
      ...
      return reply.status(204).send({ message: 'Player deleted successfully.' })
    } catch (error) {
      if (error instanceof Error && error.message === 'Player not found') {
        return reply.status(404).send({
          message: error.message,
