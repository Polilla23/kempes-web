import api from './api'
import type { RegisterClubFormData, ClubResponse, Club } from '@/types'

export class ClubService {
  // Crear un nuevo club
  static async createClub(clubData: RegisterClubFormData): Promise<ClubResponse> {
    try {
      const response = await api.post<ClubResponse>('/clubs', clubData)
  ...
  static async getClubById(id: string): Promise<Club | undefined> {
    try {
  ...
  static async updateClub(id: string, clubData: RegisterClubFormData): Promise<ClubResponse> {
    try {
      const response = await api.patch<ClubResponse>(`/clubs/${id}`, clubData)
  ...
  static async deleteClub(id: string): Promise<ClubResponse> {
    try {
      const response = await api.delete<ClubResponse>(`/clubs/${id}`)
